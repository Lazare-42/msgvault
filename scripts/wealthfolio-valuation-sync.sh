#!/usr/bin/env bash
# wealthfolio-valuation-sync.sh — Extract per-holding NAV valuations from email
# using Claude + msgvault MCP, then write nav-envelope JSON for Wealthfolio's
# NAV-healing watcher (<data-root>/nav-inbox/).
#
# Mirrors wealthfolio-order-sync.sh. Runtime state lives under
# $MSGVAULT_HOME/wealthfolio-valuations/{config.json,mcp.json,rules.md,state.json,last_run}.

set -euo pipefail

MSGVAULT_HOME="${1:?Usage: wealthfolio-valuation-sync.sh <MSGVAULT_HOME>}"
SYNC_DIR="$MSGVAULT_HOME/wealthfolio-valuations"
...
LOOKBACK_DAYS="$(jq -r '.lookback_days_on_first_run // 400' "$CONFIG_FILE")"
...
Extract per-holding NAV valuations from email for Wealthfolio NAV healing.
...
3. Extract holding-level unit values (NAV) from the newest message content, per the system rules.
4. Return strict JSON only in the exact nav-envelope format described in the system prompt.
5. Set ruleName to the matching rule's name. Map holdings by ISIN; no account id is needed.
6. If there are no valuations, return {"envelopes":[]}.
EOF

echo "Wealthfolio valuation sync: $LAST_RUN → $NOW"
...
echo "New valuation envelopes: $NEW_COUNT"
...
write_envelopes() {
    jq -c '.[]' "$TMP_FILTERED" | while IFS= read -r envelope; do
        ...
        as_of="$(printf '%s' "$envelope" | jq -r '.asOf // .sentAt // "unknown-date"')"
        ...
        out_file="$OUTBOX_DIR/nav-${stamp}_${safe_source}_${safe_rule}.json"

        # The nav-healing watcher reads version/asOf/prices and ignores the
        # provenance fields; recordKey is dropped as it is dedup-only.
        printf '%s' "$envelope" \
...
echo "Wealthfolio valuation sync complete. Updated last_run to $NOW"
