...
func TestWhatsAppStatusUsesWhatsAppAccountScope(t *testing.T) {
	...
	assertpkg.Equal(t, "15551234567@s.whatsapp.net", factoryAccount)
	assertpkg.True(t, status.Paired)
	assertpkg.True(t, status.Ready)
	assertpkg.Equal(t, "15551234567@s.whatsapp.net", status.AccountJID)
}
...
func TestSendWhatsAppMessage(t *testing.T) {
	...
	assertpkg.Equal(t, "15557654321@s.whatsapp.net", gotReq.ChatID)
	assertpkg.Equal(t, "hello", gotReq.Body)
	assertpkg.Equal(t, "req-1", gotReq.LocalRequestID)
	assertpkg.Equal(t, int64(8), result.MessageID)
...
func TestSendWhatsAppReactionAllowsEmptyEmojiToClear(t *testing.T) {
	...
	assertpkg.Equal(t, int64(42), gotReq.MessageID)
	assertpkg.Equal(t, "", gotReq.Emoji)
	assertpkg.Equal(t, "react-1", gotReq.LocalRequestID)
	assertpkg.Equal(t, int64(42), result.MessageID)
}
...
func TestGoogleDocsHandlers(t *testing.T) {
	t.Run("list forwards filters", func(t *testing.T) {
		...
		requirepkg.Len(t, files, 1, "files")
	...
	t.Run("search returns snippets", func(t *testing.T) {
		client := &mockGoogleDocsClient{
			...
			getDoc: func(_ context.Context, sourceName, documentID string, maxChars int) (*googledocs.Document, error) {
				assertpkg.Equal(t, "work", sourceName, "sourceName")
				assertpkg.Equal(t, "doc-1", documentID, "documentID")
				assertpkg.Equal(t, maxGoogleDocsMaxChars, maxChars, "maxChars")
				return &googledocs.Document{
		...
		requirepkg.Len(t, results, 1, "results")
		assertpkg.Contains(t, results[0].Snippet, "target", "snippet")
	})
...
func TestSearchMessages(t *testing.T) {
	...
	t.Run("valid query", func(t *testing.T) {
		resp := runTool[paginatedCompactMessages](t, "search_messages", h.searchMessages, map[string]any{"query": "from:alice"})
