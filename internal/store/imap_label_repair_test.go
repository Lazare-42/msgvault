...
func TestRepairIMAPSourceLabels_RemovesStrayAddOnlyLabel(t *testing.T) {
	...
	require.NoError(err)
	changed, _, err := f.store.ReconcileMessageLabels(messageID1, []int64{strayLabelID}, false)
	require.NoError(err)
...
func TestRepairIMAPSourceLabels_DryRunDoesNotWrite(t *testing.T) {
	...
	strayLabelID, err := f.store.EnsureLabel(f.source.ID, "stray-source-label", "Stray", "user")
	require.NoError(err)
	_, _, err = f.store.ReconcileMessageLabels(messageID1, []int64{strayLabelID}, false)
	require.NoError(err)
