...
func TestScopesForEmail(t *testing.T) {
	...
	assertpkg.Equal(t, ScopeSMTPOrg, orgScopes[1], "org SMTP scope")
	personalScopes := scopesForEmail("user@hotmail.com")
	assertpkg.Equal(t, ScopeIMAPPersonal, personalScopes[0], "personal scope")
	assertpkg.Equal(t, ScopeSMTPPersonal, personalScopes[1], "personal SMTP scope")
}
...
func TestTokenSource_CorrectScopeSucceeds(t *testing.T) {
	...
	requirepkg.NoError(t, m.saveToken("user@outlook.com", token, []string{ScopeIMAPPersonal, ScopeSMTPPersonal, "offline_access"}, MicrosoftConsumerTenantID))
...
func TestTokenSource_PersistedTenantOverridesManager(t *testing.T) {
	...
	requirepkg.NoError(t, m.saveToken("user@company.com", token, []string{ScopeIMAPOrg, ScopeSMTPOrg, "offline_access"}, "my-org-tenant"))
...
func TestTokenSource_PreMigrationTokenGetsTenantBinding(t *testing.T) {
	...
	requirepkg.NoError(t, m.saveToken("user@company.com", token, []string{ScopeIMAPOrg, ScopeSMTPOrg, "offline_access"}, ""))
